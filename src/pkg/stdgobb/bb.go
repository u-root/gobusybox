...
// Package stdgobb builds one busybox-like binary out of many Go command sources.
//
...
import (
	...
	"github.com/u-root/gobusybox/src/pkg/stdgobb/bbpkg"
...
func checkDuplicate(cmds []*bb.Package) error {
...
func BuildBusybox(opts *Opts) (nerr error) {
	...
	cmds, err := bbpkg.NewPackages(opts.Env, opts.CommandPaths...)
	if err != nil {
...
func writeBBMain(bbDir, tmpDir string, bbImports []string) error {
	...
	bbFset, bbFiles, _, err := bb.ParseAST("main", []string{filepath.Join(bbDir, "main.go")})
	if err != nil {
		return err
	}
	if len(bbFiles) == 0 {
	...
	// Create bb main.go.
	if err := bb.CreateBBMainSource(bbFset, bbFiles, bbImports, bbDir); err != nil {
...
// directives.
func localModules(pkgDir, bbDir string, mainPkgs []*bb.Package) (map[string]*packages.Module, error) {
...
func dealWithDeps(env golang.Environ, bbDir, tmpDir, pkgDir string, mainPkgs []*bb.Package) error {
	...
	for _, p := range localDepPkgs {
		if _, ok := seenIDs[p.ID]; !ok {
			if err := bb.WritePkg(p, filepath.Join(pkgDir, p.PkgPath)); err != nil {
